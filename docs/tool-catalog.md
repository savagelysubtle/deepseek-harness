...
Schedule compaction of older conversation history. Takes no arguments. The request is accepted immediately; the actual condensation runs right after the current turn ends, replacing older history with one summary checkpoint while recent context stays verbatim. Call it once when accumulated history is no longer needed in full; calling it again while a schedule is already pending changes nothing.
