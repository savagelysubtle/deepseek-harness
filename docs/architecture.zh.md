...
**每个会话日志一个写入者。** 命名会话的日志在同一时刻只有一个合法写入者：持有其按名称锁文件的进程。运行该会话轮次的每条路径——终端命令、脚本化运行、邮件触发的唤醒——都经由同一个 headless 入口点并获取同一把锁，因此两个竞争者干净地分出胜负：一方获取锁，另一方收到占用错误。常驻的 web 宿主进程从不写入命名会话的日志；当活跃所有者持锁时，宿主以只读方式跟随其日志（见 [packages/host/apiproxy](../packages/host/apiproxy/README.md)）。
