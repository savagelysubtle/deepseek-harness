...
**每个座位会话日志的写入者：宿主。** 命名座位会话的日志只有一个合法写入者：拥有它的 dsh 宿主进程——就像消息服务器拥有账户历史（Telegram 的云聊天、Matrix 的 homeserver）。与座位对话的每个界面——web UI、CLI、邮件触发的唤醒、将来的手机——都向宿主提交输入；宿主将其排序进日志，并把结果流式推送给所有连接的客户端。自己写会话日志的进程（一次性 headless 任务）拥有座位不共享的匿名日志。任何第二个进程都绝不向座位日志追加：两个写入者不是被竞争或调和——第二个写入者根本不存在。
