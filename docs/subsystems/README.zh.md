...
| [mailbox.md](mailbox.md) | 持久代理邮箱 seam：地址文法、存储消息生命周期、认领与落定、提供方契约与 steering 投递 |
| [memory.md](memory.md) | 工作区作用域记忆服务：项目作用域、限额、read/write/list/search 契约与面向模型的工具 |
...
| [worktree.md](worktree.md) | worktree seam：`WorktreeProvider`/`WorktreeSpawnRequest`/`WorktreeSpawnResult`/`WorktreeRow`/`WorktreeSlug`、由 seam 铸造的名字、force 门禁与本地 git provider |
