...
/**
 * Poll `observe` until it returns a defined value, or throw `onTimeout()` once
 * the deadline has passed.
 *
 * The deadline is only checked AFTER an observation attempt completes — never
 * mid-attempt — so a timeout always carries a real "I looked and it was still
 * absent" result. `vi.waitFor` instead races its own timer against the
 * callback and, if the timer wins before any attempt has settled, rejects
 * with its own generic "Timed out in waitFor!" — discarding the descriptive
 * error the callback would have thrown, exactly when a slow first attempt
 * (e.g. file I/O under CPU load) needs it most. `observe` itself may still
 * throw to fail the wait immediately without retrying (a validation error,
 * not a not-yet state) — that propagates straight out of this loop.
 */
async function pollUntil<T>(
  observe: () => Promise<T | undefined> | T | undefined,
  timeoutMs: number,
  onTimeout: () => Error,
): Promise<T> {
...
async function waitForPersistedTurnStart(
  ...
  await pollUntil(async () => {
    ...
    // latestOpenTurn throws, which propagates straight out of pollUntil
    // without retrying.
    const openTurn = log === undefined ? undefined : latestOpenTurn(log.content)
    if (openTurn === undefined || (minimumTurn !== undefined && openTurn < minimumTurn)) return undefined
    return true
  }, timeoutMs, () => {
    const detail = minimumTurn === undefined ? 'turn/start' : `turn/start at or beyond turn ${minimumTurn}`
    return new Error(`snapshot-harness: session "${sessionId}" did not persist ${detail} within ${timeoutMs}ms`)
  })
}
...
async function waitForPersistedTurnEnd(
  ...
  await pollUntil(async () => {
    const log = (await harvestSessionLogs(root)).find(candidate => candidate.id === sessionId)
    return log !== undefined && latestTurnIsClosed(log.content) ? true : undefined
  }, timeoutMs, () => new Error(`snapshot-harness: session "${sessionId}" did not persist turn/end within ${timeoutMs}ms`))
}
...
async function waitForPersistedChildTurnEnd(
  ...
  await pollUntil(async () => {
    ...
    const ready = log !== undefined && latestTurnIsClosed(log.content)
...
async function waitForPersistedGoalPhase(
  ...
  }, timeoutMs, () => new Error(`snapshot-harness: session "${sessionId}" did not persist goal phase "${phase}" within ${timeoutMs}ms`))
}
...
async function waitForPersistedInboxMessage(
  ...
  await pollUntil(async () => {
    ...
    return matched ? true : undefined
  }, timeoutMs, () => new Error(`snapshot-harness: session "${sessionId}" did not persist expected inbox message within ${timeoutMs}ms`))
}
...
async function waitForPersistedTitleAfterTurnEnd(
  ...
  await pollUntil(async () => {
    const log = (await harvestSessionLogs(root)).find(candidate => candidate.id === sessionId)
    return log !== undefined && latestTitleFollowsTurnEnd(log.content) ? true : undefined
  }, timeoutMs, () => new Error(`snapshot-harness: session "${sessionId}" did not persist session/title after turn/end within ${timeoutMs}ms`))
}
...
async function waitForPersistedEventAfterTurnEnd(
  ...
  await pollUntil(async () => {
    const log = (await harvestSessionLogs(root)).find(candidate => candidate.id === sessionId)
    return log !== undefined && latestEventFollowsTurnEnd(log.content, type) ? true : undefined
  }, timeoutMs, () => new Error(`snapshot-harness: session "${sessionId}" did not persist ${type} after turn/end within ${timeoutMs}ms`))
}
...
async function waitForWorkspaceFile(
  ...
  await pollUntil(() => existsSync(target) ? true : undefined, timeoutMs,
    () => new Error(`snapshot-harness: workspace file "${path}" did not appear within ${timeoutMs}ms`))
}
