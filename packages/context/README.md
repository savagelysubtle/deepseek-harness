...
Product plugins that add model-visible request context without defining a tool. `agent-instructions` is included by the default `dsh-agent-spine-demo` bundle and can be disabled through bundle config; `time-context`, `tmux-context`, `session-reference`, and `context-pressure` are opt-in.
...
| [`context-pressure/`](context-pressure/README.md) | Context-window pressure warnings | — |
