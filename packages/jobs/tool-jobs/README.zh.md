...
机器来源的所有者——session header 携带 `origin: 'subagent'` 的 agent，即受托派的子代理——适用更大的独立预算 `machineOwnerWakeBudget`（默认 16），重置规则相同。子代理无法求助（其审批策略在托派时即已固定），因此耗尽预算不会无声降级：最后一次唤醒会明确指示它用 memory 工具持久化状态并转入安静，此后到达的通知才改为注入且不再开启轮次。用户撰写的领取会同时恢复预算并重新武装这轮收尾。
...
| `machineOwnerWakeBudget` | `16` | 机器来源所有者（`origin: 'subagent'`）的同一上限，降级前附带最后一轮明确的收尾唤醒 |
