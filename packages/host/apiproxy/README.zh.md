...
`mailbox.publish` 是面向非 dsh 调用方（WebCEO 控制台）的线路准入入口：网关先校验寻址——完整的 `address`，或 `namespace`+`name`，绝不允许两种形式并存；格式错误的地址与不在任何已挂载桥花名册内的地址在写入任何内容之前即以 `mailbox-rejected` 拒绝——随后经注册表默认提供方（[`@deepseek-ai/dsh-mailbox`](../../mailbox/mailbox/README.md)）发布，并立即通过[桥](../../mailbox/bridge/README.md)的 `publishAndWake` 路由，使新邮件在本次调用内即唤醒目标，而非等待下一个轮询。响应携带存储分配的 `messageId` 与一个处置字段：路由在本次唤醒中把消息送入某个会话收件箱（存活的 steer 优先，或驻留锁下冷恢复）则为 `delivered`；驻留被他处持有或认领批次已满排在其前则为 `queued`。终态路由失败（`unknown-address`、未组合 session-persistence）同样以携带记录原因的 `mailbox-rejected` 应答，调用方绝不会把沉默误读为送达。缺少邮箱注册表或任何桥的组合会得到明确指明缺口的拒绝。
...
- **worktree seam 契约由消费端声明**——由于 seam 包正在并行构建，`src/worktree-seam.ts` 在本地声明了 seam 服务接口、其拒绝错误以及 `ctx.worktree` 的 Context 合并；落地真实包的整合提交会把这些声明替换为 import，消费端其余部分不得改变。在此之前，wire 词汇（`WorktreeHandle`、`WorktreeRow`、spawn 输入）即声明记录。
