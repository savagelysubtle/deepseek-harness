...
The Streamable HTTP config carries an explicit `auth: { mode: 'oauth', scope?, clientId?, redirectPort? }` block, and the SDK's `OAuthClientProvider` is implemented over `ctx.credentials`:
