...
const NO_MODEL_EXPERIENCE_SECTION: Readonly<Record<string, string>> = {
  ...
  'packages/util/port-allocator': 'The package only allocates kernel ports and builds session environment objects; model-facing consumers own any rendered use.',
}
...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/session/session-turn-status': { kind: 'none', reason: 'The turnStatus unit folds already-logged turn boundaries into a client-facing read model and registers nothing model-facing.' },
  ...
  'packages/agui/ag-ui': { kind: 'none', reason: 'The server projects the session log onto outbound AG-UI sockets; it registers nothing model-facing.' },
  'packages/mailbox/mailbox': { kind: 'indirect', reason: 'The seam stores and leases durable messages; consuming delivery surfaces (the bridge, the headless runner) own every model-visible turn.' },
  'packages/session/named-sessions': { kind: 'none', reason: 'The package derives session ids and lock artifacts; it registers nothing model-facing.' },
